...
def main() -> int:
    ...
    print(
        f"Catalog mode: "
        f"{os.getenv('PRODUCT_CATALOG_MODE', 'synthetic')}"
    )
    ...
    if not os.getenv("GROQ_API_KEY"):
        print(
            "Note: GROQ_API_KEY is not set; "
            "LLM/image extraction will be unavailable."
        )

    if not os.getenv("NEO4J_PASSWORD"):
        print(
            "Note: NEO4J_PASSWORD is not set; "
            "Neo4j-backed rules may be unavailable."
        )
